...
# Load 3D Gaussian Splatting model from checkpoint
...
# Train the ID module for a single object and evaluate
def pretrain_single_object(
    ...
    # Initialize Identification Module with DINO backbone
    backbone_type = "dino"
    id_module = (
    ...
    # Check for existing checkpoint to resume or skip training
    start_iterations = 0
    ...
    # Compute mean up vector from training cameras
    model_up_np = np.mean(
        np.asarray(
...
# Generate sampling rays from the Gaussian model
...
# Wrapper to run evaluation for a single object
...
def main():
    ...
    # Iterate through all experiments found in the directory
